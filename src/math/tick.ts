...
export function getTickDataFromUrlData(ticks: Tick[]): TickData[] {
  const tickdatas: TickData[] = []
  for (const tick of ticks) {
    const td: TickData = {
      index: Number(BigInt.asIntN(64, BigInt(tick.index.bits)).toString()),
      sqrtPrice: new BN(tick.sqrt_price),
      liquidityNet: new BN(BigInt.asIntN(64, BigInt(tick.liquidity_net.bits)).toString()),
      ...
      rewardersGrowthOutside: [
        new BN(tick.rewarders_growth_outside[0]),
        new BN(tick.rewarders_growth_outside[1]),
        new BN(tick.rewarders_growth_outside[2]),
      ],
    }
    tickdatas.push(td)
  }
